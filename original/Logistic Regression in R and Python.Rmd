...
```{r echo=FALSE}
library(knitr)
opts_chunk$set(message=F, comment=NULL)
```
...
$$L(\theta)_R = -\frac{\text{log likelihood}}{nobs} + \lambda * \text{penalty}$$

其中$nobs$代表样本量。惩罚项如果是L2惩罚，则是$\frac{1}{2} \parallel \theta \parallel_2^2$，其中**不包含截距项**。如果是L1惩罚，就是$|\theta|_1$.
...
$$L(\theta)_{python} = -C*\text{log likelihood} + \frac{1}{2}\parallel \theta \parallel_2^2$$
与R语言的区别在于，这里的惩罚项里**包含截距项**。另外参数设置上，这里的$C$相当于$\lambda$，都是作为一个权衡似然函数和惩罚项的参数。
...
增大`intercept_scaling=100000`的结果：
```{r engine='python'}
from sklearn import linear_model
from sklearn import datasets
iris = datasets.load_iris()
...
R语言中添加截距项的结果：
```{r}
logit = glmnet(iris.x, iris.y, family="binomial", alpha=0, lambda=1/100, standardize=F, thresh =1e-8, intercept = T)
coef(logit)
```
